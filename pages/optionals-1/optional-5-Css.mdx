...
- `flex-end`: es lo opuesto a `flex-start`, mostrará los elementos al final de una fila o columna.
...
- `space-evenly`: Similar a `space-around`, pero aplicará márgenes igualitarios entre cada elemento y al comienzo y al final del contenedor.
...
En CSS, las propiedades personalizadas (también conocidas como variables) son entidades definidas por autores de CSS que contienen valores específicos que se pueden volver a utilizar en un documento o en otro. Se establecen mediante la notación de propiedades personalizadas `--nombre: valor;` y se acceden mediante la función `var()`.
