...
`querySelectorAll` funciona de la misma forma que `querySelector` pero en vez de devolver únicamente el primer elemento que coincida con el selector devolverá un array con todos los elementos que coincidan. Ejemplo:
...
```javascript
const a = document.querySelector('#linkCurso') // Obtengo el elemento a cuyo id es 'linkCurso'

a.setAttribute('href', 'https://cursofrontend.vercel.app/') // Seteo el atributo href del elemento para que redireccione a la página principal del Curso

a.href = 'https://cursofrontend.vercel.app/' // Equivalente al anterior pero más corto
```
