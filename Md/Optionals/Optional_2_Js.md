...
let agustin = new Usuario("Agustín", "ArenasAgustin");

console.log(juan.introduccion()); // Mi nombre es Juan, mi usuario de Github es juan.perez.
console.log(agustin.introduccion()); // Mi nombre es Antonio, mi usuario de Github es atralice.
```
...
> var Agus = new Persona('Agus', 'Arenas', 'Cordoba');
...
> function Alumno(nombre, apellido, ciudad, curso) {
    ...
    this.curso = curso;
    this.empresa = 'Arenas Corp.';
  }

> var pedro = new Alumno('Pedro', 'Picapiedra', 'La Rioja', 'Desarrollo Web')
...
> pedro.saludar()
< Uncaught TypeError: pedro.saludar is not a 'function'
// que paso?
```
...
> var Peter = new Alumno('Peter','Parker','New York','Curso web')

> Peter.saludar()
< 'Soy Peter de New York'
```
